            ...
            <div>
                {PlatformTabs.map((item) => (
                    <div key={item.id} className={tab === item.id ? "" : "hidden"}>
                        <h1 className="text-white font-semibold text-4xl md:text-5xl my-10">{item.heading}</h1>
                        <div className='flex max-sm:flex-col-reverse justify-center gap-12'>
                            <div className='flex flex-col justify-between gap-4'>

                                <div className="text-white max-w-[500px] min-h-[200px] text-start text-lg font-semibold">
                                    {item.content}
                                </div>
                                ...
                                <div className="text-white text-start">
                                    {item.points.map((point, index) => (
                                        <div key={index} className='flex gap-4 mb-2'>
                                            <Image className="inline h-[25px]" src={tick} alt='tick' width={25} />
                                            <h3>{point}</h3>
                                        </div>
                                    ))}
                                </div>

                                <button className="inline-block max-md:mt-12 mt-4 text-white border border-1 border-[rgb(37,40,55)] font-bold hover:bg-[rgb(37,40,55)] max-w-[300px] p-4 rounded-md">
                                    {item.button}
                                </button>
                            </div>

                            <Image className="bg-white text-lg font-semibold" src={item.image} width={600} alt={item.id} />
                        </div>
                    </div>
                ))}
            </div>
